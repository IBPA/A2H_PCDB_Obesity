...
def load_best_rgs_pipeline(path_rgs_dir, model_type=None):
    gs_result = pd.read_csv(f"{path_rgs_dir}/regressors/grid_search_results.csv")
    
    if model_type:
        best_model_config = gs_result[gs_result["model"]==model_type].reset_index(drop=True).iloc[0]
    ...
    with open(
        f"{path_rgs_dir}/regressors/rgs_{best_model_name}_{best_od}_{best_mvi}_{best_fs}"
        f"_{best_os}.pkl",
        'rb'
    ) as f:
...
def _get_fs(method_name):
    ...
    """
    # print("Using scaling method : ", method_name)
