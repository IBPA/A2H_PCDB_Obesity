################################################################################
# SEMAGLUTIDE DIO BASELINE WEIGHT ANALYSIS: LMER — MICE ONLY
#
# Estimand: does preclinical baseline body weight (before treatment) predict the
# translational gap in semaglutide DIO studies, restricted to mouse models?
#
...
cat("================================================================================\n")
cat("SEMAGLUTIDE DIO BASELINE WEIGHT ANALYSIS: LMER — MICE ONLY\n")
cat("================================================================================\n\n")
...
screen_results <- lapply(fe_terms, function(term) {
  ...
  if (is.factor(vals) || is.character(vals)) {
    fit_aov <- tryCatch(
      aov(df_sema$pre_wt_treat_g ~ droplevels(factor(vals))),
      error = function(e) NULL
    )
    if (is.null(fit_aov)) {
    ...
    r <- abs(cor(as.numeric(vals), df_sema$pre_wt_treat_g, use = "complete.obs"))
...
if (wt_term %in% rownames(coefs_final)) {
  ...
  if (!is.na(ref_p) && ref_p >= 0.05) {
    cat("-> NO SIGNIFICANT baseline weight effect (Semaglutide DIO — Mice)\n\n")
  } else {
...
if (!is.null(model_re_pmcid)) {
  ...
  vc2 <- tryCatch(vcovCR(model_re_pmcid, cluster = df_sema$pmcid, type = "CR2"),
                  error = function(e) { cat("  CR2 (pmcid) failed:", e$message, "\n"); NULL })
  vc1 <- tryCatch(vcovCR(model_re_pmcid, cluster = df_sema$pmcid, type = "CR1"),
                  error = function(e) NULL)
  if (!is.null(vc2)) {
...
if (!is.null(model_re_nct)) {
  ...
  vc2 <- tryCatch(vcovCR(model_re_nct, cluster = df_sema$NCT_Number, type = "CR2"),
                  error = function(e) { cat("  CR2 (NCT) failed:", e$message, "\n"); NULL })
  vc1 <- tryCatch(vcovCR(model_re_nct, cluster = df_sema$NCT_Number, type = "CR1"),
                  error = function(e) NULL)
  if (!is.null(vc2)) {
...
## --- Diagnostic plots ---
plot_file <- "sema_dio_baselinewt_mice_lmer_robust_diagnostics.pdf"
...
if (wt_term %in% rownames(coefs_final)) {
  ...
  write.csv(result_row, "sema_dio_baselinewt_mice_lmer_robust_summary.csv", row.names = FALSE)
  cat("Results saved to: sema_dio_baselinewt_mice_lmer_robust_summary.csv\n\n")
  print(result_row)
}
...
cat("================================================================================\n")
cat("SEMAGLUTIDE DIO BASELINE WEIGHT LMER ROBUST ANALYSIS (MICE ONLY) COMPLETE\n")
cat("================================================================================\n")
