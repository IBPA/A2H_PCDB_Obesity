################################################################################
# LIRAGLUTIDE DIO DURATION ANALYSIS: LMER — MICE ONLY
#
# Estimand: does preclinical treatment duration predict the translational gap
# in liraglutide DIO studies, restricted to mouse models?
#
...
#   suppression bias.
#
#   Backward selection of fixed effects using lmerTest (Kuznetsova et al., 2017)
#   with F-tests (Satterthwaite) at α = 0.1:
#     - drop1() for fixed effects (ML models, REML = FALSE)
...
# as DIO are excluded (same as lira_dio_duration_mice_lmer_analysis.R):
...
cat("================================================================================\n")
cat("LIRAGLUTIDE DIO DURATION ANALYSIS: LMER — MICE ONLY\n")
cat("================================================================================\n\n")
...
screen_results <- lapply(fe_terms, function(term) {
  ...
  if (is.factor(vals) || is.character(vals)) {
    ## η = sqrt(SS_between / SS_total) from one-way ANOVA
    fit_aov <- tryCatch(
      aov(df_lira$pre_dur_days ~ droplevels(factor(vals))),
      error = function(e) NULL
    )
    if (is.null(fit_aov)) {
    ...
    r <- abs(cor(as.numeric(vals), df_lira$pre_dur_days, use = "complete.obs"))
...
## All covariates retained — no automated elimination (VIF < 5 for all terms)
selected_fe   <- fe_terms
...
## =============================================================================
## STEP 4: DURATION EFFECT — Wald t-test and LRT
## =============================================================================
...
if (dur_term %in% rownames(coefs_final)) {
  ...
  if (!is.na(ref_p) && ref_p >= 0.05) {
    cat("-> NO SIGNIFICANT duration effect (Liraglutide DIO — Mice)\n\n")
  } else {
...
## Helper: extract duration CRSE row from coef_test output
crse_dur_row <- function(ct, beta_ref) {
  if (is.null(ct) || !dur_term %in% rownames(ct))
    return(list(se = NA, df = NA, p = NA, ci_l = NA, ci_u = NA))
  se_v <- ct[dur_term, "SE"]
  df_v <- ct[dur_term, "df_Satt"]
  p_v  <- ct[dur_term, "p_Satt"]
  list(
    se   = se_v,
    df   = df_v,
    p    = p_v,
...
if (!is.null(model_re_pmcid)) {
  beta_pmcid <- summary(model_re_pmcid)$coefficients[dur_term, "Estimate"]
  cat(sprintf("  β (pmcid-RE model): %.5f  (crossed-RE: %.5f, diff = %+.5f)\n\n",
...
pmcid_cr2 <- crse_dur_row(crse_pmcid_cr2, beta_pmcid)
pmcid_cr1 <- crse_dur_row(crse_pmcid_cr1, beta_pmcid)
...
if (!is.null(model_re_nct)) {
  beta_nct <- summary(model_re_nct)$coefficients[dur_term, "Estimate"]
  cat(sprintf("  β (NCT-RE model): %.5f  (crossed-RE: %.5f, diff = %+.5f)\n\n",
...
nct_cr2 <- crse_dur_row(crse_nct_cr2, beta_nct)
nct_cr1 <- crse_dur_row(crse_nct_cr1, beta_nct)

## --------------------------------------------------------------------------
## Unified comparison table — duration term only
## --------------------------------------------------------------------------

cat("DURATION TERM — UNIFIED SE COMPARISON\n")
cat("--------------------------------------\n")

reml_se_v  <- coefs_final[dur_term, "Std. Error"]
reml_p_v   <- coefs_final[dur_term, "Pr(>|t|)"]
reml_ci_l  <- ci_final[dur_term, "2.5 %"]
reml_ci_u  <- ci_final[dur_term, "97.5 %"]

cat(sprintf("  β = %.5f  (per raw day of preclinical treatment)\n\n", beta))
cat(sprintf("  %-36s  SE = %.5f,  95%% CI [%.5f, %.5f],  p = %s %s\n",
...
fmt_crse_row <- function(label, r) {
  if (is.na(r$se)) {
    ...
    cat(sprintf("  %-36s  SE = %.5f,  95%% CI [%.5f, %.5f],  p = %s %s  (df = %.1f)\n",
...
## --- Diagnostic plots ---
plot_file <- "lira_dio_duration_mice_lmer_robust_diagnostics.pdf"
...
if (dur_term %in% rownames(coefs_final)) {
  result_row <- data.frame(
    ...
    REML_SE           = round(se, 5),
    REML_CI_L         = round(ci_l, 5),
    REML_CI_U         = round(ci_u, 5),
    REML_Wald_p       = pval,
    LRT_chi2          = round(lrt_chi, 4),
    LRT_df            = lrt_df,
    LRT_p             = lrt_p,
    CR2_pmcid_SE      = round(cr2_se_v,     5),
    CR2_pmcid_df      = round(cr2_df_v,     2),
    CR2_pmcid_CI_L    = round(cr2_ci_l,     5),
    CR2_pmcid_CI_U    = round(cr2_ci_u,     5),
    CR2_pmcid_p       = cr2_p_v,
    CR2_pmcid_ratio   = round(cr2_se_v / se, 3),
    CR2_NCT_SE        = round(nct_cr2_se_v,  5),
    CR2_NCT_df        = round(nct_cr2_df_v,  2),
    CR2_NCT_CI_L      = round(nct_cr2_ci_l,  5),
    CR2_NCT_CI_U      = round(nct_cr2_ci_u,  5),
  ...
  write.csv(result_row, "lira_dio_duration_mice_lmer_robust_summary.csv", row.names = FALSE)
  cat("Results saved to: lira_dio_duration_mice_lmer_robust_summary.csv\n\n")
  print(result_row)
}
...
cat("================================================================================\n")
cat("LIRAGLUTIDE DIO DURATION LMER ROBUST ANALYSIS (MICE ONLY) COMPLETE\n")
cat("================================================================================\n")
